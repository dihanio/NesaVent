...
const createOrder = async (req, res) => {
  try {
    ...
    // Reserve stock saat order dibuat
    for (const item of items) {
      const ticketType = event.tiketTersedia.id(item.tipeTiket);
      if (ticketType) {
        ticketType.stokTersisa -= item.jumlah; // Kurangi stok tersedia
        ticketType.stokPending = (ticketType.stokPending || 0) + item.jumlah; // Track pending
      }
    }
    await event.save();
...
const updateOrderToPaid = async (req, res) => {
  try {
    ...
    if (order) {
      ...
      // Note: Stock already reduced on order creation
      // Note: Tickets will be generated by Midtrans webhook

      res.json({ order: updatedOrder, message: 'Payment will be processed by Midtrans' });
    } else {
      res.status(404).json({ message: 'Order tidak ditemukan' });
    }
  } catch (error) {
