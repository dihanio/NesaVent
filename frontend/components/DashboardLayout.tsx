...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  const menuItems: MenuItem[] = user?.role === 'admin' ? adminMenuItems : user?.role === 'mitra' ? mitraMenuItems : userMenuItems;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Sidebar - Desktop */}
      <aside
        className={`fixed top-0 left-0 h-full bg-white border-r-2 border-blue-100 shadow-xl z-40 hidden md:block ${!isInitialLoad ? 'transition-all duration-300' : ''} ${
          ...
          {menuItems.map((item) => {
            ...
            return (
              <button
                key={item.href}
                onClick={() => router.push(item.href)}
                className={`flex flex-col items-center gap-1 px-2 py-2 rounded-xl min-w-14 ${
                  isActive
                    ? 'text-blue-600'
                    : 'text-gray-500'
                }`}
      ...
      {/* Main Content */}
      <main
        className={`${isMobile ? '' : (!isInitialLoad ? 'transition-all duration-300' : '')} ${
      ...
      <aside
        className={`fixed top-0 left-0 h-full bg-white shadow-2xl z-40 md:hidden w-72 ${
